...
| Id    | Applies to  | Title |
| ----- | ----------- | ----- |
| U-067 | running     | A digest pin lands with its bump bot, and the bot's runs are bounded |
...
| U-066 | running     | The governance check's sanction covers its existence, not its size |
...
| U-062 | bootstrap   | How much review the code gets is asked, not left to the implementer |
