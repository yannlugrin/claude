...
# --- manage.py: the one thing here that writes outside the repository ------
# Safe by default with a listable set of dangerous acts, so `rules`. It links
# assets into `~/.claude/`, or into any directory `--target` names, so its
# reach is the home directory rather than this clone. Two acts are worth a
# prompt: removing links, and replacing a symlink this repository does not own.
# Everything else — `status`, a plain `enable` — is the tool doing its job.
#
# `./manage.py` and `python3 manage.py` both resolve here: the engine matches
# on the basename, and reads the script argument of a known interpreter.

MANAGE = Tool(
    name="manage.py",
    rules=(
        Rule("ask", ("disable",), "this removes links from the target directory"),
        Rule("ask", (), "`--force` replaces a symlink this repository does not own",
             flags=frozenset({"--force"})),
    ),
)

# --- rm: dangerous by default, with the rebuildable directories granted -----
# The other way round, because here the *safe* side is the finite one: this
# repository's throwaway state is the virtualenv and the tool caches, all
# rebuilt by `just setup` or by the next run. Everything else `rm` could reach
# is the working tree, the git database, or something outside this clone.

RM = Tool(
    name="rm",
    gated_reason="deleting anything but the rebuildable tool directories",
    known_flags=frozenset(
        {"-r", "-R", "--recursive", "-f", "--force", "-rf", "-fr", "-Rf", "-fR",
         "-v", "--verbose", "-d", "--dir", "--"}
    ),
    grants=(
        # One grant, three locations: the virtualenv, the linter cache, and a
        # `__pycache__` anywhere in the tree. The last is a regex because
        # `under` matches a *location*, and this one has no fixed place —
        # `under` still resolves the path first, so `.venv/../skills` does not
        # sneak in through a prefix that only looks right.
        Grant(operands=under(".venv", ".ruff_cache",
                             re.compile(r"(?!\.\.|/)(?:.*/)?__pycache__(?:/.*)?$"))),
    ),
)

TOOLS: dict[str, Tool] = registry(GIT, DOCKER, MANAGE, RM)
...
CASES: tuple[tuple[str, str], ...] = (
    ...
    # --- this repository -----------------------------------------------------
    # the everyday loop stays quiet
    ("just check", "silent"),
    ("just check changed", "silent"),
    ("just verify", "silent"),
    (".venv/bin/pre-commit run --all-files", "silent"),
    ("./manage.py status", "silent"),
    ("python3 manage.py status", "silent"),
    ("./manage.py enable specify", "silent"),
    ("./manage.py enable --all", "silent"),
    ("rm -rf .venv", "silent"),
    ("rm -rf .ruff_cache", "silent"),
    ("rm -rf skills/specify/__pycache__", "silent"),
    ("rm -rf ../other-project/__pycache__", "ask"),  # outside this clone
    ("rm -rf /tmp/__pycache__", "ask"),
    # manage.py reaches outside this clone, so its two destructive acts ask
    ("./manage.py disable specify", "ask"),
    ("python3 manage.py disable --all", "ask"),
    ("./manage.py enable --force specify", "ask"),
    ("./manage.py --target ../app/.claude enable --force specify", "ask"),
    # deleting anything that is not rebuildable tooling
    ("rm -rf skills/specify", "ask"),
    ("rm -rf .venv/../skills", "ask"),  # resolves back out
    ("rm README.md", "ask"),
    ("rm -rf ~/projects", "ask"),
)
